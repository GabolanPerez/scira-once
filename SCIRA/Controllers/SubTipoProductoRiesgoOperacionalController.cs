...
{
    [Authorize]
    [Access(Funcion = "STProductoRO", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class SubTipoProductoRiesgoOperacionalController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_sub_tipo_producto_riesgo_operacional.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_sub_tipo_producto_riesgo_operacional c_sub_tipo_producto_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_sub_tipo_producto_riesgo_operacional.esta_activo = true;
                db.c_sub_tipo_producto_riesgo_operacional.Add(c_sub_tipo_producto_riesgo_operacional);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            ViewBag.id_producto_riesgo_operacionalL = Utilidades.DropDown.AmbitoRiesgoOperacional(c_sub_tipo_producto_riesgo_operacional.id_producto_riesgo_operacional);
            return View(c_sub_tipo_producto_riesgo_operacional);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_sub_tipo_producto_riesgo_operacional c_sub_tipo_producto_riesgo_operacional = db.c_sub_tipo_producto_riesgo_operacional.Find(id);
            if (c_sub_tipo_producto_riesgo_operacional == null)
            {
                return HttpNotFound();
            }
            return View(c_sub_tipo_producto_riesgo_operacional);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_sub_tipo_producto_riesgo_operacional c_sub_tipo_producto_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_sub_tipo_producto_riesgo_operacional.esta_activo = true;
                db.Entry(c_sub_tipo_producto_riesgo_operacional).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_sub_tipo_producto_riesgo_operacional);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            c_sub_tipo_producto_riesgo_operacional.esta_activo = false;
        ...
        public ActionResult AuxInfoP(HttpPostedFileBase archivo)
        {
            ...
            try
            {
                if (System.IO.File.Exists(Server.MapPath("~/App_Data/BDEIAuxInfo/subTipoProducto.pdf")))
                    System.IO.File.Delete(Server.MapPath("~/App_Data/BDEIAuxInfo/subTipoProducto.pdf"));
                if (System.IO.File.Exists(Server.MapPath("~/App_Data/BDEIAuxInfo/subTipoProducto.png")))
                    System.IO.File.Delete(Server.MapPath("~/App_Data/BDEIAuxInfo/subTipoProducto.png"));

                archivo.SaveAs(Server.MapPath("~/App_Data/BDEIAuxInfo/subTipoProducto." + ext));
                return RedirectToAction("Index");
            }
            catch (Exception e)
            {
