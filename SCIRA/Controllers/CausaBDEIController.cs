...
{
    [Authorize]
    [Access(Funcion = "CABDEI", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class CausaBDEIController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_causa_bdei.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_causa_bdei c_causa_bdei)
        {
            if (ModelState.IsValid)
            {
                c_causa_bdei.esta_activo = true;
                db.c_causa_bdei.Add(c_causa_bdei);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(c_causa_bdei);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_causa_bdei c_causa_bdei = db.c_causa_bdei.Find(id);
            if (c_causa_bdei == null)
            {
                return HttpNotFound();
            }
            return View(c_causa_bdei);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_causa_bdei c_causa_bdei)
        {
            if (ModelState.IsValid)
            {
                c_causa_bdei.esta_activo = true;
                db.Entry(c_causa_bdei).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_causa_bdei);
        ...
        public ActionResult Delete(int? id, string redirect = null)
        {
            ...
            //var r_bdei = db.k_bdei.Where(b => b.id_causa_bdei == id).ToList();
        ...
        {
            c_causa_bdei c_causa_bdei = db.c_causa_bdei.Find(id);
            c_causa_bdei.esta_activo = false;
            try
            {
                db.SaveChanges();
            }
