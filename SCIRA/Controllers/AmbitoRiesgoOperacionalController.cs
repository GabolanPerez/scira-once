...
{
    [Authorize]
    [Access(Funcion = "AmbitoRO", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class AmbitoRiesgoOperacionalController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_ambito_riesgo_operacional.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_ambito_riesgo_operacional c_ambito_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_ambito_riesgo_operacional.esta_activo = true;
                db.c_ambito_riesgo_operacional.Add(c_ambito_riesgo_operacional);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(c_ambito_riesgo_operacional);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_ambito_riesgo_operacional c_ambito_riesgo_operacional = db.c_ambito_riesgo_operacional.Find(id);
            if (c_ambito_riesgo_operacional == null)
            {
                return HttpNotFound();
            }
            return View(c_ambito_riesgo_operacional);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_ambito_riesgo_operacional c_ambito_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_ambito_riesgo_operacional.esta_activo = true;
                db.Entry(c_ambito_riesgo_operacional).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_ambito_riesgo_operacional);
        ...
        public ActionResult Delete(int? id, string redirect = null)
        {
            ...
            //var r_proceso_riesgo_operacional = db.c_proceso_riesgo_operacional.Where(b => b.id_ambito_riesgo_operacional == id).ToList();

            ////creamos un objeto de tipo RegistrosRelacionadosViewModel para cada uno de estos elementos y lo incluimos en la lista RR
            //if (r_proceso_riesgo_operacional.Count > 0)
            //{
            //    foreach (var sctr in r_proceso_riesgo_operacional)
            //    {
            //        RegistrosRelacionadosViewModel rr = new RegistrosRelacionadosViewModel();
            //        rr.nb_catalogo = "Proceso de Riesgo Operacional";
            //        rr.cl_registro = sctr.cl_proceso_riesgo_operacional;
            //        rr.nb_registro = sctr.nb_proceso_riesgo_operacional;
            //        rr.accion = "Delete";
            //        rr.controlador = "ProcesoRiesgoOperacional";
            //        rr.id_registro = sctr.id_proceso_riesgo_operacional.ToString();
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            Utilidades.DeleteActions.DeleteAmbitoROObjects(c_ambito_riesgo_operacional, db);
            c_ambito_riesgo_operacional.esta_activo = false;
