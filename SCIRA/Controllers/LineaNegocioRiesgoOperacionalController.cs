...
{
    [Authorize]
    [Access(Funcion = "LNRO", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class LineaNegocioRiesgoOperacionalController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_linea_negocio_riesgo_operacional.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_linea_negocio_riesgo_operacional c_linea_negocio_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_linea_negocio_riesgo_operacional.esta_activo = true;
                db.c_linea_negocio_riesgo_operacional.Add(c_linea_negocio_riesgo_operacional);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            ViewBag.id_categoria_linea_negocio_riesgo_operacionalL = Utilidades.DropDown.CategoriaLineaNegocioRiesgoOperacional(c_linea_negocio_riesgo_operacional.id_categoria_linea_negocio_riesgo_operacional);
            return View(c_linea_negocio_riesgo_operacional);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_linea_negocio_riesgo_operacional c_linea_negocio_riesgo_operacional = db.c_linea_negocio_riesgo_operacional.Find(id);
            if (c_linea_negocio_riesgo_operacional == null)
            {
                return HttpNotFound();
            }
            return View(c_linea_negocio_riesgo_operacional);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_linea_negocio_riesgo_operacional c_linea_negocio_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_linea_negocio_riesgo_operacional.esta_activo = true;
                db.Entry(c_linea_negocio_riesgo_operacional).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_linea_negocio_riesgo_operacional);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            c_linea_negocio_riesgo_operacional.esta_activo = false;
