...
{
    [Authorize]
    [Access(Funcion = "TipoRO", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class TipoRiesgoOperacionalController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_tipo_riesgo_operacional.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_tipo_riesgo_operacional c_tipo_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_tipo_riesgo_operacional.esta_activo = true;
                db.c_tipo_riesgo_operacional.Add(c_tipo_riesgo_operacional);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(c_tipo_riesgo_operacional);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_tipo_riesgo_operacional c_tipo_riesgo_operacional = db.c_tipo_riesgo_operacional.Find(id);
            if (c_tipo_riesgo_operacional == null)
            {
                return HttpNotFound();
            }
            return View(c_tipo_riesgo_operacional);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_tipo_riesgo_operacional c_tipo_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_tipo_riesgo_operacional.esta_activo = true;
                db.Entry(c_tipo_riesgo_operacional).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_tipo_riesgo_operacional);
        ...
        {
            c_tipo_riesgo_operacional c_tipo_riesgo_operacional = db.c_tipo_riesgo_operacional.Find(id);
            Utilidades.DeleteActions.DeleteTipoROObjects(c_tipo_riesgo_operacional, db);
            c_tipo_riesgo_operacional.esta_activo = false;
            try
            {
                db.SaveChanges();
            }
