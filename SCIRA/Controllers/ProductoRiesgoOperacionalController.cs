...
{
    [Authorize]
    [Access(Funcion = "ProductoRO", ModuleCode = "MSICI005")]
    [CustomErrorHandler]
    public class ProductoRiesgoOperacionalController : Controller
    {
        private SICIEntities db = new SICIEntities();

        public ActionResult Index()
        {
            return View(db.c_producto_riesgo_operacional.Where(r => r.esta_activo ?? false).ToList());
        }
        ...
        [NotOnlyRead]
        public ActionResult Create(c_producto_riesgo_operacional c_producto_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_producto_riesgo_operacional.esta_activo = true;
                db.c_producto_riesgo_operacional.Add(c_producto_riesgo_operacional);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(c_producto_riesgo_operacional);
        }
        #endregion
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_producto_riesgo_operacional c_producto_riesgo_operacional = db.c_producto_riesgo_operacional.Find(id);
            if (c_producto_riesgo_operacional == null)
            {
                return HttpNotFound();
            }
            return View(c_producto_riesgo_operacional);
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit(c_producto_riesgo_operacional c_producto_riesgo_operacional)
        {
            if (ModelState.IsValid)
            {
                c_producto_riesgo_operacional.esta_activo = true;
                db.Entry(c_producto_riesgo_operacional).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_producto_riesgo_operacional);
        ...
        {
            c_producto_riesgo_operacional c_producto_riesgo_operacional = db.c_producto_riesgo_operacional.Find(id);
            Utilidades.DeleteActions.DeleteProductoROObjects(c_producto_riesgo_operacional, db);
            c_producto_riesgo_operacional.esta_activo = false;
            try
            {
                db.SaveChanges();
            }
