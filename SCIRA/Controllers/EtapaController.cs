...
{
    [Authorize]
    [Access(Funcion = "Etapas", ModuleCode = "MSICI000")]
    [CustomErrorHandler]
    public class EtapaController : Controller
    {
        ...
        [NotOnlyRead]
        public ActionResult Create([Bind(Include = "id_etapa,cl_etapa,nb_etapa")] c_etapa c_etapa)
        {
            if (ModelState.IsValid)
            {
                db.c_etapa.Add(c_etapa);
                db.SaveChanges();
        ...
        public ActionResult Edit(int? id)
        {
            ...
            c_etapa c_etapa = db.c_etapa.Find(id);
            if (c_etapa == null)
            {
                return HttpNotFound();
            }
        ...
        [ValidateAntiForgeryToken]
        [NotOnlyRead]
        public ActionResult Edit([Bind(Include = "id_etapa,cl_etapa,nb_etapa")] c_etapa c_etapa)
        {
            if (ModelState.IsValid)
            {
                db.Entry(c_etapa).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(c_etapa);
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            Utilidades.DeleteActions.DeleteEtapasObjects(c_etapa, db);

            db.c_etapa.Remove(c_etapa);
            try
            {
                db.SaveChanges();
            }
